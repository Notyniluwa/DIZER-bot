...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
  ...
  if (!SESSION_ID) return console.log('Please add your session to SESSION_ID env !!');
...
async function connectToWA() {
  ...
  const { PREFIX: prefix, AUTO_READ_STATUS, ownerNumber } = config;
  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      if (lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut) {
        await connectToWA();  // Recursive call to reconnect
      }
      ...
      const up = `Dizer-md connected successfully ✅\n\nPREFIX: ${prefix}`;
      await conn.sendMessage(ownerNumber + "@s.whatsapp.net", { image: { url: `https://telegra.ph/file/600fb1c5551ef67e24d76.jpg` }, caption: up });
    }
  });
  ...
  conn.ev.on('messages.upsert', async (mek) => {
    ...
    if (mek.key && mek.key.remoteJid === 'status@broadcast' && AUTO_READ_STATUS === "true") {
